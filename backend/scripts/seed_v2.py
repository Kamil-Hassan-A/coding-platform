...
# Allow running as: python scripts/seed_v2.py from backend/
...
# Seed directly from the new unified dataset payload kept beside this script.
        ...
        for level_key, level_payload in levels.items():
            ...
            for difficulty in CANONICAL_DIFFICULTIES:
                ...
                for question in bucket:
                    ...
                    description = to_str(question.get("description")) or "No description provided"
                    ...
                    raw_sample = question.get("test_cases") or question.get("sample_test_cases") or []
                    sample_test_cases = normalize_test_cases(raw_sample)
                    ...
                    if question_type == "framework":
                        raw_starter_files = question.get("starter_files") or []
                        entry_point = str(question.get("entry_point") or "test_solution.py").strip()
                        test_harness = question.get("test_harness") or ""

                        files = []
                        for f in raw_starter_files:
                        ...
                        # Add test harness as its own file if provided
                        if test_harness and entry_point:
                            # Only add if not already in files
                            existing_paths = {f["path"] for f in files}
                            if entry_point not in existing_paths:
                                files.append({
                                    "path": entry_point,
                                    "content": test_harness
                                })

                        starter_code = {
                            "files": files,
                            "entry_point": entry_point,
                            "readonly_files": [entry_point]
                        } if files else None
                    else:
                        starter_code = normalize_starter_code(
                            question.get("starter_code"), language
                        )

                    raw_options = question.get("options")
                    raw_correct_index = question.get("correct_option_index")
                    raw_correct_letter = question.get("correct_option")

                    if question_type == "mcq" and raw_options:
                        # Convert index to letter if needed
                        if raw_correct_index is not None:
                            correct_option = ["A","B","C","D"][int(raw_correct_index)]
                        elif raw_correct_letter:
                            correct_option = str(raw_correct_letter).strip().upper()
                        else:
                            correct_option = None
                        type_data = {
                            "options": raw_options,
...
def run_seed(input_json: Path) -> None:
    ...
    try:
        ...
        print("Seeding complete (seed_v2.py).")
