...
export default function ProblemPanel({ problem, language }: Props) {
  ...
  hideSampleTestCases?: boolean;
}

export default function ProblemPanel({ problem, hideSampleTestCases }: Props) {
              ...
              {sampleCases.map((tc, i) => (
                <div key={i} className='rounded-xl border border-[#eef0f2] bg-[#f8f9fa] p-5'>
                  <div className='flex flex-col gap-4 md:flex-row md:gap-6'>
                    <div className='flex-1'>
                      <div className='mb-2 text-[11px] font-bold text-[#aaa]'>INPUT</div>
                      <pre className='m-0 overflow-x-auto whitespace-pre-wrap break-words rounded-md border border-[#eee] bg-white p-2.5 font-mono text-[13px] text-[#333]'>
                        {tc.input || " (empty) "}
                      </pre>
                    </div>
                    <div className='flex-1'>
